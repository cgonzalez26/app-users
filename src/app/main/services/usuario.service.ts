...
export class UsuarioService {
  ...
  destroy(usuario: Usuario){
    return this.http.delete('./usuarios'+ usuario.id).pipe(
      map((response) => { return response})
    );  
  }  

  update(usuario: Usuario){
    return this.http.put('./usuarios'+ usuario.id, usuario).pipe(
      map((response) => { return response})
    );  
  }  
  ...
  getUsuario(usuario: Usuario) : Observable<Usuario>{
    return this.http.get<Usuario>('./usuarios'+ usuario.id).pipe(
      map((response) => { return response})
    );
  } 
}
